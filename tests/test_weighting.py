...
def test_uncertainty_equals_fixed_at_init_and_gets_grad():
    ...
    # STAGE 1: Now has 5 tasks (itc, itm, smap, box, anomaly)
    assert set(w.weights()) == {"itc", "itm", "smap", "box", "anomaly"}
...
def test_dwa_equal_weights_until_history_then_upweights_slow_task():
    ...
    k = w.weights()
    # STAGE 1: Now has 5 tasks, so sum = 5.0 instead of 3.0
    assert abs(sum(k.values()) - 5.0) < 1e-4          # softmax * K sums to K
